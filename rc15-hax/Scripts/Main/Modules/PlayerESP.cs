...
public class PlayerESP : HaxModules {
    ...
    void DrawESP() {
        ...
        foreach (Rigidbody rigidbody in HaxObjects.Rigidbodies.Objects) {
            ...
            Vector2 coordinatesTextPosition = new Vector2(rigidbodyScreenPosition.x - halfWidth, rigidbodyScreenPosition.y - halfHeight - this.TextBottomPadding);
            GUIHelper.DrawLabel(coordinatesTextPosition, $"{Mathf.RoundToInt(distanceFromRigidbody).ToString()}m");
        }
    }
}